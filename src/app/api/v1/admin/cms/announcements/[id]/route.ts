...
/** Single announcement (Doc 09 §6.5). Venue-scoped; unknown ids answer 404. */
...
export const PUT = withApiHandler<{ id: string }>(async (request, { requestId, params }) => {
  ...
  await cms.updateAnnouncement(params.id, venueConfig.id, input, session.adminId)
...
export const DELETE = withApiHandler<{ id: string }>(async (request, { requestId, params }) => {
  ...
  await cms.retireAnnouncement(params.id, venueConfig.id, session.adminId)
  return apiSuccess({ retired: true }, { requestId })
})
